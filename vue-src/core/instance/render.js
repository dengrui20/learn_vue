...
export function initRender (vm: Component) {
  vm._vnode = null // the root of the child treenew
  ...
  /*将createElement函数绑定到该实例上。这样我们就可以得到正确的上下文渲染*/
...
export function renderMixin (Vue: Class<Component>) {
  ...
  Vue.prototype._render = function (): VNode {
    ...
    vm.$vnode = _parentVnode  // 获取占位vnode 也就是父vnode
    // render self
    /*渲染*/
    let vnode
    try {
      ...
      /* istanbul ignore else */
      //当 render 函数遭遇错误时，提供另外一种渲染输出。其错误将会作为第二个参数传递到 renderError 例如
      // new Vue({
      //   render (h) {
    ...
    // set parent
    vnode.parent = _parentVnode  // 生成的vnode 指向 占位vnode
    return vnode
  }
