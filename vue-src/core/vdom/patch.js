...
import attrs from "../../platforms/web/runtime/modules/attrs";
import klass from "../../platforms/web/runtime/modules/class";
import events from "../../platforms/web/runtime/modules/events";
import domProps from "../../platforms/web/runtime/modules/dom-props";
import style from "../../platforms/web/runtime/modules/style";
import transition from "../../platforms/web/runtime/modules/transition";
  ...
  */
  ...
  function createElm (vnode, insertedVnodeQueue, parentElm, refElm, nested) {
    ...
    if (isDef(tag)) {
      ...
      // 如果vnode 存在namespace 则创建一个namespace元素
      // 创建一个真实的dom
      vnode.elm = vnode.ns
      ...
      if (__WEEX__) {
        ...
        // 如果存在子节点 递归调用createElm
        // createChildren -> createElm
        // 先插入子元素 一层层往上insert 最终将整个节点添加到body
        createChildren(vnode, children, insertedVnodeQueue)
        if (isDef(data)) {
          // 调用dom的钩子 渲染 vnode 对应的真实dom vnode.elm 真实dom的data 如 class id event style 等
          // 先插入子节点 后插入父节点
          invokeCreateHooks(vnode, insertedVnodeQueue)
        }
        // 最终调用insert 方法
  ...
  function createComponent (vnode, insertedVnodeQueue, parentElm, refElm) {
    let i = vnode.data
    if (isDef(i)) {
      const isReactivated = isDef(vnode.componentInstance) && i.keepAlive // 关于keep-alive
      // 判断vnode.data是否存在hook   并且hook是否存在init方法
      if (isDef(i = i.hook) && isDef(i = i.init)) {
        // 执行data.hook.init()方法  此方法在 core/vdom/create-components中定义 定义在componentVNodeHooks对象上
  ...
  function initComponent (vnode, insertedVnodeQueue) {
    ...
    // 在 createComponent    i(vnode, false /* hydrating */, parentElm, refElm)
    // 执行了Vue._init() lifecycle -> updata -> patch 将$el返回给了实例
  ...
  function invokeCreateHooks (vnode, insertedVnodeQueue) {
    ...
    if (isDef(i)) {
      ...
      if (isDef(i.insert)) insertedVnodeQueue.push(vnode) // 将vnode 添加到队列
    }
  }
  ...
  function invokeInsertHook (vnode, queue, initial) {
    ...
    if (isTrue(initial) && isDef(vnode.parent)) {
      ...
      for (let i = 0; i < queue.length; ++i) {
        // 执行队列里面各队vdom的insert钩子 // 在里面调用mounted钩子
  ...
  // 通过函数科柯里化  将平台的差异化在此函数里做完处理后 返回处理后的函数  不需要每次执行patch执行方法的时候通过if 判断
    ...
    if (isUndef(oldVnode)) {
      ...
      if (!isRealElement && sameVnode(oldVnode, vnode)) {
        ...
        createElm(
          ...
          // 如果旧元素 正在leaveing 中 不可插入元素
          // 只会在 transition 和 keepalive 及高阶组件中发生
