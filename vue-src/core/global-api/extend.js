...
export function initExtend (Vue: GlobalAPI) {
  ...
  */
  ...
  Vue.extend = function (extendOptions: Object): Function {
    ...
    /*如果构造函数中已经存在了该cid，则代表已经extend过了，直接返回  缓存优化*/
    ...
    */
    const Sub = function VueComponent (options) {
      this._init(options)  
      // this._init 是调用父类Vue 上的_init方法, 
      // 所以在初始化_init的过程中所有的vm实例或者this指向都是指向的当前组件VueComponent的实例而不是Vue的实例
      // vueComponent拥有了和vue完全一样的init过程
...
/*处理计算属性，给计算属性设置defineProperty并绑定在当前vueComponent的实例vm上*/
function initComputed (Comp) {
  const computed = Comp.options.computed
  // 先挂载在vueComponent的构造器上
  // 但是这个时候依赖的值都是undefined,所以Computed的值都是undefined
  // 然后再_init的过程中再次initComputed,直接new 一个watcher的时候默认求值一次
  // 获取到的用户设置的getter获取到了正确的值, 就不会执行再次挂载的过程了
  // if (!(key in vm)) {
